  ...
  const getRootDirectory = async () => {
    ...
    if (user) {
      ...
      if (userId[0]) {
        ...
        const { data: rootDirectoryData, error: rootDirectoryError } =
          await supabase.from("directories").select().eq("dir_id", rootDirId);
        ...
        <>
          {dirs?.map((dir) => {
            ...
            return (
              <div
                key={dir_id}
                className="dir-icon-container"
                onClick={() => getDirectoryDetails(`${dir_id}`)}
              >
